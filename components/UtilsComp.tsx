...
// ─── Stats summary bar ───────────────────────────────────────────────────────
export function StatsBar({ apps }: { apps: AP[] }) {
  const counts = apps.reduce((acc, application) => ({
    ...acc,
  ...
  return (
    <div className="grid grid-cols-5 gap-4 rounded-xl border border-border bg-card p-4 mb-6">
